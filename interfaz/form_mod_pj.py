...
class FormularioModPJ(Formulario):
    ...
    def cargarwidgets(self):
        ...
        ent_nom = tk.Entry(frame_nombre, textvariable=self.nombre_pj)
        ent_nom.pack(side=tk.RIGHT, fill=tk.X, padx=2, expand=True)
        ent_nom.insert(tk.END,self.pj.nombre)
        ...
        ent_jug = tk.Entry(frame_jugador, textvariable=self.jugador)
        ent_jug.pack(side=tk.RIGHT, fill=tk.X, padx=2, expand=True)
        ent_jug.insert(tk.END, self.pj.jugador)
        ...
        ent_desc = tk.Entry(frame_descripcion, textvariable=self.descripcion)
        ent_desc.pack(side=tk.TOP, fill=tk.BOTH, padx=2, expand=True)
        ent_desc.insert(tk.END, self.pj.descripcion)
        ...
        for caracteristica in cons.caracteristicas.keys():
            ...
            self.dic_caracteristicas_spin[caracteristica].set(self.pj.caracteristicas[caracteristica])
            if fila <1: fila+=1
            else:
                columna+=1
                fila=0
        ...
        items.BotonEstandar(frame_texto, "Modificar personaje", self.modificar_personaje
        ).pack(
            side=tk.TOP,
            fill=tk.X,
            pady=5
        )
        borrar = items.BotonEstandar(frame_texto, "Borrar personaje", self.eliminar_personaje)
    ...
    def modificar_personaje(self):
        ...
        if self.controlador.modificar_personaje(
            self.id_antiguo,
            self.nombre_pj.get(),
            "pj",
            self.jugador.get(),
    ...
    def eliminar_personaje(self):
        ...
        if not mb.askokcancel("PELIGRO", f"¿Seguro que quieres borrar a {self.pj.nombre}?"):
